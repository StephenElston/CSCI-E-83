...
## Welcome to Second Half of CSCI E-83!    

Plan going forward:    

- Week 8: Introduction to Linear Models    
- Week 9: Linear Models Part 2 - Categorical data and regularization     
- Week 10: Nonlinear Regression Models and Classifiers     
- Week 11: Time Series Models       
- Week 12: Clustering Models    
- Week 13: Bayes MCMC methods    
- Week 14: Hierarchical models    
- Submit Graduate Independent Projects  
...
- In this lesson we take a **frequentist view** of the linear modfel    
...
- $\hat{y}$ is the vector of $n$$ dependent variables or labels we seek to predict   

- $A$ is the $n$ x $p$ **model matrix**      
   - Defines the structure of the model    
   - $p$ columns are values of the predictor variables or features 
   - $n$ rows of traning cases   
...
- $\epsilon$ is the vecctor representing **prediction error**   
   - The $n$ **residuals** 
...
How do we copute a best fit model for this relatioship 
...
- Direct matrix inverse algorithm has complexity $O(n^2)$, so inefficient   
...
$$\mathcal{L}(\vec{b}, \sigma^2) = \prod_{i=1}^n \frac{1}{(2 \pi \sigma2)^{1/2}} exp \Big\{ -\frac{1}{2 \sigma^2} \big(y_i - x_i^T \vec{b}  \big)^2 \Big\}$$

- The log-likilihood is then:     
...
- Inverting the covariance matrix is a big improvement over a naive approach, it still requires taking large matrix inverse at scale. $p > 1,000$     
...
Outliers are a persistent problem with statistical and macine learing models   
...
- But, may be of interest    
   - Depending on application can be the **most interesting values**!!    
...
- Outliers can be hard to detect   
   ...
   - Ofen find by influence on model   
...
## Print the model coefficient
print('Intercept = %4.3f  Parial Slope = %4.3f' % (ols_model_ol._results.params[0], ols_model_ol._results.params[1]))
...
- We can study the response of estimators to outiers using an **influence function**   
...
- Approximately linear influence near point estimate      
   - So nearly unbiased near the point estiamte   
...
ols_model_huber = sm_api.RLM(sim_data_ol.y, sim_data_ol.x, M=sm.robust.norms.HuberT()).fit()
# Add predicted to pandas data frame
...
```{python, echo=FALSE}
## Print the model coefficient
#print('Intercept = %4.3f  Parial Slope = %4.3f' % (ols_model_huber._results.params[0], ols_model_huber._results.params[1]))
