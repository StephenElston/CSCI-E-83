...
- First consider how the Normal likelihood can be written in terms of $\vec{b}$ with $n$ feature observations $\vec{x}_i$ and $n$ labels $y_i$:
...
- For a fixed $\sigma^2$ one can see that the **log-likelihood is maximized by minimizing the least squared error:**    
...
- Set the first derivative os sum of square errors to zero

$$\frac{\partial\ SS(\vec{b})}{\partial\ \vec{b}} = 2 \sum_{i=1}^n x_i\big(y_i - \vec{b}^T x_i   \big) = 0$$
Or, in matrix form:     
...
- The normal equations are a maximum likelihhood estimator   
