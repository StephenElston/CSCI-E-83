...
```{python}
ax = sns.scatterplot('time_decimal', 'medSoldPriceSqft', data=housing) 
...
```{python}
ax = sns.scatterplot('time_decimal', 'log_medSoldPriceSqft', data=housing)
_=ax.set_xlabel('Time', fontsize=14)
...
```{python}
ax = sns.scatterplot('time_decimal', 'log_medSoldPriceSqft', data=housing, alpha=0.1)
_=ax.set_xlabel('Time', fontsize=14)
_=ax.set_ylabel('Log Median Sale Price', fontsize=14)
_=ax.set_title('Log median sale prices vs. time', fontsize=14)
plt.show()
...
```{python}
ax = sns.scatterplot('time_decimal', 'log_medSoldPriceSqft', data=housing, alpha=0.1,  s=2)
_=ax.set_xlabel('Time', fontsize=14)
_=ax.set_ylabel('Log Median Sale Price', fontsize=14)
_=ax.set_title('Log median sale prices vs. time', fontsize=14)
plt.show()
...
> **Exercise:** In the previous exercise you created a hexbin plot of vehicle weight vs. city MPG using various grid sizes. Now, create a 2-d KDE plot for the same variables. What aspects of the relationship are more apparent on these plots? Which aspects of this relationship are harder to see? 
...
> **Exercise:** Recreate the scatter plot matrix from the previous exercise, but use the tittered `S4` variable. How does using a bit of jitter change your perception of the relationships between `S4` and the other variables? In particular can you tell differences between the two sexes of the patients? 
