...
- In most machine learning we assume data samples are **independent identically distributed** (iid)
...
- Joseph Fourier and Siemon Poisson worked on time series problems in the 19th Century    
...
- Developed the **autoregressive (AR)** model to account for series correlation    
...
- The ARIMA model is a main focus here     
...
- Long short term memory (LSTM) models; more next quarter
...
Time series are expressed as a time ordered sequence of values $(x_1, x_2, x_3, \dots, x_n)$    

- We work with **discrete samples** in time order   
- Sample interval $\Delta t$ is fixed   
- Time measured from start of series $(0, \Delta t, 2 \Delta t, \ldots,n \Delta t)$   
- Or, time measured within an **interval**   
...
- Can express values, $(w_1, w_2, w_3, \dots, w_n)$, of a white noise series as:   
...
- Many time series models require stationary    
...
- The **autocorrelation function (ACF)** is measured between itself and the series at each lag    
...
- In other words, the autocorrelation of any time series at **lag zero** equals 1.0   
- Autocorrelation at each lag has values in the range $-1 \ge \rho \ge 1$   
...
- To compute the partial autocorrelation to lag $k$:   
  ...
  * Remove the linearly predictable component of the time series from the autocorrelations   
...
def acf_pacf_plot(ts):
    ...
    _=splt.plot_pacf(ts, lags = 40, ax=ax[1])
    plt.show()
...
shvalue of a series in one time step:    
$$w_t = x_t - x_{t-1}$$   
- The value of the next value in the random walk is then:   
$$x_t = x_{t-1} + w_t$$  
...
- Partial autocorrealtion at one lag   
...
White noise series is Normally distributed
...
- The distribution of even a white noise series with trend is not-Normal   
...
- The PACF exhibits periodic behavior   
...
Example of STL decomposition of time series with linear trend and seasonal effect   
...
$$x_t = \phi_1 x_{t-1} + \phi_2 x_{t-2} \dots \phi_p x_{t-p} + w \epsilon_t\\
where\\
...
- The value of $y_t$ is a weighted sum $k$ previous values plus an error term    
...
$$
\mathbb{E}(X_t) = \phi_t \mathbb{E}(X_{t-1}) + \mathbb{E}(\epsilon_t) \\
or \\
\mu = c + \phi_t \mu + 0 \\
therefore \\
\mu = \frac{c}{1 - \phi_t^2 }
...
- Notice that value of the second is wi the confidence interval    
...
x_1,\ 0,\ 0\\
x_2,\ \epsilon_{1},\ 0\\
x_3,\ \epsilon_{2},\ \epsilon_{1}\\
x_4,\ \epsilon_{3},\ \epsilon_{2}\\
\vdots,\ \vdots,\ \vdots\\
x_{t-2}, \epsilon_{t-3}, \epsilon_{t-4}\\
x_{t-1}, \epsilon_{t-2}, \epsilon_{t-3}\\
x_t, \epsilon_{t-1},\epsilon_{t-2}
\end{bmatrix}
$$
...
$$
\phi_p(X) \tilde{\phi}_P(X^*) \Delta^d \Delta^D y_t =  A(t) + \theta_q(X) \tilde{\theta}_Q(X^*)
$$


- $\phi_p(X)$ and $\tilde{\phi}_P(X^*)$ are the AR polynomials non-seasonal and seasonal terms   
- $\theta_q(X)$ and $\tilde{\theta}_Q(X^*)$ are the MA polynomials non-seasonal and seasonal terms  
- $\Delta^d$ and $\Delta^D$ are the non-seasonal and seasonal differencing operators      
...
SARIMAX model can include exogenous variables   

$$
y_t = \beta_t z_t + \mu_t 
$$
$$
\phi_p(X) \tilde{\phi}_P(X^*) \Delta^d \Delta^D \mu_t =  A(t) + \theta_p(X) \tilde{\theta}_P(X^*)
$$

- Time series model for latent variable, $\mu_t$    
- $\mu_t$ acts as the intercept term for the regression model for $y_t$    
...
- BIC penalizes the score function for the complexity of the model by $2\ k$    
...
Example of SARIMAX model of order (0.1.1)(0,1,1,12) for monthly electric production series    
