...
- The more negative the values of the second partial derivatives, the greater the curvature of the log-likelihood. This means that the log-likelihood likelihood function has a narrower range. The narrow peak implies that the information on the parameter value for the model is high.    
...
> - Compare the parameter values estimated with the gradient descent algorithm with the values computed using closed form soluton. Are you estimated parameters close to the closed form solutions? If not, something is likely wrong with your gradient descent algorithm.    
...
Examine your plot and answer these questions:   
> - Examine the plots of the parameter values and norm of the gradient vs. iteration. Can you see evidence that one parameter is easier to fit than the other?   
> - Notice the instability fo the norm of the gradient neart the convergence point. Can you account for this instability arrising from the mini-batch sampling? 
> - Compare the parameter values estimated with the gradient descent algorithm with the values computed using closed form soluton. Are you estimated parameters close to the closed form solutions? If not, something is likely wrong with your gradient descent algorithm.    
