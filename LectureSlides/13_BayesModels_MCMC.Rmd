...
- The **prior distribution** encodes the information we have in advance about the model parameters    
   - Simple cases use a **conjugate distribution** of the likelihood     
...
```{r MCMC_500SamplesBivariateNormal, out.width = '40%', fig.cap='MCMC samples from bivariate Normal distribution', fig.align='center', echo=FALSE}
...
- The Hamiltonian, $\mathtt{H}(q,p)$, of the system **must remain constant** over the sample space
...
$$p(q,p) = \frac{1}{Z} e^{-\frac{\mathtt{H}(q,p)}{kT}}$$

- Where:    
   * $Z =$ normalization or **partition coefficient**   
   * $k =$ scale parameter    
   * $T =$ temperature    
...
2. Simulate $q_t$ and $p_t$ for L time intervals with $\epsilon$ time steps, using the coupled differential equations   
3. Get $q_L$ as the new proposed state     
4. Apply the M-H acceptance criteria to $q_L$

- Must find good hyperparameter values of $L$ and $\epsilon$ to **tune** the algorithm    
  * Change too small, search is only local    
  * Change too large, search makes large jumps   
   
- Hard to find good hyperparameter values, $L$ and $\epsilon$, in practice      
...
- NUTS improves on HMC     
  ...
  * Equilibrium point determines $L$ and $\epsilon$ automatically  
  * Eliminates tuning    
...
4. Prior predictive checks - are the priors reasonable for the problem?  
5. MCMC sample the model      
