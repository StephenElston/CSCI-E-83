...
**Hypothesis testing** seeks to answer the question, are the differences in the distributions of certain variables **statistically significant**?      
...
- Upper and lower one-sided CIs can be expreseed:    

$$1 - \alpha  = P(x \le Lower\ CI)\ \\ and \\\ \alpha = P(x \le Upper\ CI)$$
...
- For the lower tail of the Normal distribution, the confidence interval is defined:   

$$1 - \alpha = P\big( -\infty \le x \le lower\ CI \big)$$

- Or, for the upper tail of a random variable, the confidence interval is defined:      

$$\alpha = P\big( -\infty \le x \le upper\ CI \big)$$
...
- Once the cutoff value has been set and the test statistic computed, interpret the results:  
  - If the test statistic does not exceed the cutoff value, **accept the null hypothesis**   
