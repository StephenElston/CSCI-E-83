...
- Likelihood of sample from a **generating process** with a **parametric probability distribution**, $f(\mathbf{X} | \theta)$       
...
- Now, for each observation, $x_i$, in $\mathbf{X} = x_1, x_2, \ldots, x_n$, the probability is just $f(x_i |\ \mathbf{\theta})$    
...
- For the set of observations, $\mathbf{X} = [x_1, x_2, \ldots, x_n]$, the likelihood is the product of these probabilities:   
...
Computing gradient is computed for each dimension of the model weight vector   

```{eval=FALSE}
proceedure compute_expected_grad(weights, cases):   
  for each dimension:
      grad[dimension] = grad(weights, cases)
  return grad
```  

The forgoing proceedure can be vectorized or parallelized   
...
Example; Binomial likelihood with $p=0.5$ showing extreme gradients near limits, $p=0$ and $p=1$
...
def plot_likelihood_1(sample_dist, df, num_samples, start, stop, p, linestyle):
    ...
    for i,samps in enumerate(num_samples): 
        ...
        ## Plot the likelihood    
        _=ax.plot(X, y, linewidth=1, label= str(samps) + ' samples', linestyle=linestyle[i] )

    ## Add annotions to plot
    ax.vlines(p, ymin=min(y), ymax=0.0, linewidth=1, linestyles='dotted')
    ax.set_ylabel('Log-likelihood')
    ax.set_xlabel('x')
    ax.set_title('Log-likelihood for different numbers of samples')
    ax.legend()
    plt.show()         
...
Example; Binomial likelihood with $p=0.5$ showing high Fisher information near limits, $p=0$ and $p=1$
