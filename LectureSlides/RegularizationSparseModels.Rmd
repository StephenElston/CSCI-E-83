...
$$\Delta y^2 = E \Big[ \big( Y - \hat{f}(X) \big)^2 \Big]$$
$Y =$ the label vector     
...
$$\Delta y^2 = \Big( E[ \hat{f}(X)] - \hat{f}(X) \Big)^2 + E \Big[ \big( \hat{f}(X) - E[ \hat{f}(X)] \big)^2 \Big] + \sigma^2\\
...
- $\Big( E[ \hat{f}(X)] - \hat{f}(X) \Big)^2$, the expected value of the difference between the model output and the expected model output is the **bias** of the model    
  - For **unbiased model**, $E \Big[ \hat{f}(X)] - \hat{f}(X) \Big] = 0$
...
- For $p \times p$ dimensional matrix there are $p$ eigenvalues and orthogonal eigenvalue    

- But, there is no guarantee that the $p$ values of $\lambda$ are unique     
   - If columns of $A$ are colinear, there are $< p$ unique eigenvectors      
...
The condition number of the covariance is $\sim 72$
