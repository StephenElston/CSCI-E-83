...
- Instead, define a **link function**, known as the **logit function** for **log likelihood ratio** $\lambda$     

$$\lambda = log \Bigg\{ \frac{\theta}{1-\theta} \Bigg\}$$

- After a bit of algebra we find the exponential relationship with $y$, the **Binary response**:
...
$$\lambda = log \Bigg\{ \frac{\theta}{1-\theta} \Bigg\} = A \vec{b}$$    
for model matrix $A$ and coefficient vector $\vec{b}$

- Given estimate of $\lambda$, the **log likelihood ratio**, compute $\theta$    

$$\theta = \frac{1}{(1 + e^{-\lambda})} = \frac{1}{1 + e^{-A \vec{b}}}$$

The **link function** for Binomial response!
