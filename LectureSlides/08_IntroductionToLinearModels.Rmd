...
- Week 13: Hierarchical Bayesian models        
...
- Human interpretability is of great importance for models used for critical decisions    
   - Health care   
...
- Linear models are scaleable and interpretable     
...
- AdaBoost algorithms are go-to models for prediction on tabular data   
    - Known to give superior predicitons     
...
Let's start with a simulated data set with one predictor and one response variable   
...
We are assuming that the **error, $\epsilon$, is only attributable to the dependent variable**, $y$         
...
- $\frac{1}{p} A^TA$ is the **covariance matrix** for the data set     
...
- **Model coefficients**, with an hypothesis test on the statistical significance  

- **F-statistic** and **Prob (F-statistic)** are a measure of the significance of the model against a **null model that does not explain the data**.     

- [**Omnibus**](https://en.wikipedia.org/wiki/Omnibus_test), which is a test on the likelihood ratio between the model and a null model     

- The $R^2$ and **adjusted $R^2$** indicate the sum of squares explained:   
  - The **number of observations**, the $df_{SST}$    
  - The **degrees of freedom residuals**, $df_{SSR}$    
  -  $df_{SST} - df_{SSR} =$ number of model coefficients     
  
- [**Jarque-Bera**](https://en.wikipedia.org/wiki/Jarque%E2%80%93Bera_test) statistic is a test on the skewness and krutosis of the residuals    

- The **Condition number** a measure of how well defined the solution is to the system of linear is      
...
## Example - Fitting Centered Model
...
- Influence tappers to 0 for extreme outliers    
