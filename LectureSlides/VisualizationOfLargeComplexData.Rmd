...
- **Aesthetics:** used to increase the number of projected dimenstioons    
...
g = sns.PairGrid(diabetes.drop('Sex', axis=1), hue='sex_categorical', palette="Set2", height=1.3)
_=g.map_upper(sns.regplot, order=1, truncate=True, scatter_kws={'s':0.5})
...
- Grid defined by categorical variable for row and column    
...
with sns.plotting_context(font_scale=0.1):
    g = sns.FacetGrid(bike_share_df, col="Weather", col_order=weather.values(), row="month", row_order=calendar.month_abbr[1:], height=1.3, aspect=1.5)
